  ...
  <div class="list-page">
    ...
    <el-form ref="form" label-position="left" class="w-1000 form" :model="form" :rules="rules" label-width="94px">
      <el-row :gutter="20">
        <el-col :span="12">
          <el-form-item label="企业名称" prop="companyName">
            <el-input v-model="form.companyName" placeholder="请输入要打印证照的企业名称" />
          </el-form-item>
        </el-col>
        <el-col :span="12">
      ...
      <template v-if="search">
        <el-table ref="singleTable" :data="printList" highlight-current-row style="width: 100%;padding-top: 30px;">
          ...
          <el-table-column property="companyName" label="企业名称" width="300" align="center" />
          <el-table-column property="name" label="证件名称" width="300" align="center" />
          <el-table-column label="操作" align="center">
            <template slot-scope="{ row }">
              <div class="text-center" style="color:#409EFF;cursor:pointer" @click="handlePrint(row.imgUrl)">打印</div>
            </template>
          </el-table-column>
        </el-table>
    ...
    <el-dialog :visible.sync="showDialog" width="506px" close-on-click-modal close-on-press-escape :show-close="false">
      <div class="dialog-wrap text-center">
        ...
        <div v-if="type && device != 'MD'" class="print-tips">如果<span>{{ second }}</span>S内未操作，系统将自动退出</div>
...
import { getXNLicenseInfo } from '@/api/index'
...
export default {
  ...
  data() {
    ...
    return {
      ...
      rules: {
        ...
        // ],
        // companyName: [
        //   { message: '请填写单位名称', trigger: 'change' }
        // ],
        // phone: [
  ...
  created() {
    ...
    if (localStorage.getItem('companyName')) {
      ...
      this.form.creditCode = localStorage.getItem('creditCode')
      this.handleSearch()
    }
    // this.setTimer()
  },
  ...
  methods: {
    ...
    handleSearch() {
      // getXNLicenseInfo(inData).then(data => {
      ...
      this.$loading({ text: '首次查询时间较长，请耐心等待' })
      getXNLicenseInfo(data).then(res => {
        this.$loading().close()
        if (res.state === 'successful') { // 获取成功
          ...
          this.printList = [
            { companyName: this.form.companyName, name: '食品经营许可证', imgUrl: outData.imgUrl_is },
            { companyName: this.form.companyName, name: '食品经营许可证(副本)', imgUrl: outData.imgUrl_vice }
          ]
    ...
    handlePrint(imgUrl) {
      if (this.form.companyName && this.form.creditCode) {
        localStorage.setItem('companyName', this.form.companyName)
        localStorage.setItem('creditCode', this.form.creditCode)
      }
      ...
      if (this.device === 'MD'|| this.$utils.getLocalItem('tId').includes('MD')) {
        ...
        this.path = location.origin + getPrefix() + '/open-platform/' + imgUrl
        console.log(this.path)
        setTimeout(() => {
          fullScreenPDF.contentWindow.print()
        }, 500)
        ...
        window.document.body.innerHTML = `<img ref="printImg" id="printContent" class="printImg-img" src="${location.origin + getPrefix() + '/open-platform/' + imgUrl}"/>`
        // 延迟1秒打印，让图片加载完成
        setTimeout(() => {
    ...
    //   console.log(imgUrl)
    //   getLicenseInfo(imgUrl).then(resp => {
    //     this.$loading().close()
