<template>
  <div class="list-page">
      ...
      <template v-if="search">
        <div>
          <el-table ref="singleTable" :data="printList" highlight-current-row style="width: 100%;padding-top: 30px;">
            ...
            <el-table-column property="name" label="证件名称" width="400" align="center" />
    ...
    <el-dialog :visible.sync="showDialog" width="506px" :close-on-click-modal="false" :close-on-press-escape="false" :show-close="false">
      <div class="dialog-wrap text-center">
        ...
        <div v-if="type && !device" class="print-tips">如果<span>{{ second }}</span>S内未操作，系统将自动退出</div>
        <div v-if="type && !device" class="btn-group" style="padding: 20px 0 56px;">
          <el-button type="primary" @click="handleGoHome">返回首页</el-button>
          <el-button @click="logOut">立即退出</el-button>
        </div>
        <div v-if="type && device && !finished">打印失败？点击<span class="reload" @click="showDialog = false">重新打印</span> </div>
        <div v-if="type && device && finished" class="btn-group" style="padding: 20px 0 56px;">
          <el-button type="primary" @click="handleGoHome">返回首页</el-button>
          <el-button @click="logOut">立即退出</el-button>
        </div>
        <div v-if="type && device && !finished " class="btn-group" style="padding: 20px 0 56px;">
    ...
    <div>
      <iframe id="fullScreenPDF" class="fullScreenPDF" :src="path" frameborder="0" width="0" height="0"></iframe>
    </div>
    <div v-if="isShow" class="preview-box">
      ...
      (1——1)
        ...
        <div class="right-container-pre">
          ...
          <div>
            <span>注册日期</span>
            <span class="dateNumber" style="margin-left:70px;">{{ printData.year }}</span>
      ...
      <div style="text-align: center;"><el-button @click="screenShot1">打印正本</el-button></div>
      <!-- 个体户副本 -->
      <div ref="preview2" class="license-box">
        ...
        <div class="right-container">
          ...
          <div>
            <span>注册日期</span>
            <span class="dateNumber" style="margin-left:12px;">{{ printData.year }}</span>
            <span class="dateWord">年</span>
            <span class="dateNumber">{{ printData.monter }}</span>
            <span class="dateWord">月</span>
            <span class="dateNumber">{{ printData.date }}</span>
      ...
      <div style="text-align: center;"><el-button @click="screenShot2">打印副本</el-button></div>
      <!-- 企业正本 -->
      ...
      <div style="text-align: center;"><el-button @click="screenShot3">打印正本</el-button></div>
      <!-- 企业副本 -->
      ...
      <div style="text-align: center;"><el-button @click="screenShot4">打印副本</el-button></div>
    </div>
  </div>
</template>
...
import { getPrintList, getLicenseInfo, uploadResult, trustAgent, saveTakeRecord } from '@/api/index'
...
export default {
  ...
  methods: {
    ...
    handleSearch() {
      ...
      this.$refs['form'].validate((valid) => {
        if (valid) {
          const data = {
            ...this.form,
            device: this.$utils.getLocalItem('device') ? this.$utils.getLocalItem('device') : 'MD',
            creditCode: ''
          }
    ...
    handlePrint() {
      ...
      getLicenseInfo(this.ident, this.form.role === '法定代表人').then(resp => {
        this.$loading().close()
        if (resp.state === 'successful') {
          ...
          if (this.$utils.getLocalItem('curArea').district === '硚口区' && printData.address && printData.address.includes('自主申报')) {
          ...
          if (this.device === 'ZW' || this.$utils.getLocalItem('tId').includes('ZW')) {
            const info = {
              size: 'A3',
              ...
              birthday:	printData.entEstDateF.replace('-', '年').replace('-', '月') + '日', // yyyy年mm月dd日
    ...
    saveRecord(printData) {
      const data = { companyName: this.form.companyName, // 企业名称
