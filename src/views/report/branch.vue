  ...
  <div class="companyMessage">
    ...
    <div class="companyMessageMain">
      ...
      <el-form ref="ruleForm" :model="form" :rules="rules" class="report_form">
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item ref="隶属企业名称" label="隶属企业名称" prop="隶属企业名称">
              <el-input v-model.trim="form['隶属企业名称']" placeholder="请输入" clearable></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item ref="隶属公司统一社会信用代码" label="隶属公司统一社会信用代码/注册号" prop="隶属公司统一社会信用代码">
              <el-input v-model.trim="form['隶属公司统一社会信用代码']" placeholder="请输入" clearable></el-input>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
    ...
    return {
      ...
      handleSetDisabled: {
        disabledDate(time) { // 返回禁用日期
          return time.getFullYear() > Number(this.$utils.getItem('reportYear')[0]) || time.getFullYear() < this.$utils.getItem('reportYear')[0]
        }
      },
      draftNumber: '', // 草稿箱ID
      name: '', // 办事主体
      ...
      rules: {
        ...
        '隶属企业名称': [
          { required: true, message: '请输入隶属企业名称', trigger: 'change' }
        ],
        '隶属公司统一社会信用代码': [
          { required: true, message: '请输入隶属公司统一社会信用代码', trigger: 'change' },
          { validator: validateField('credit-code') }
        ],
        '企业通讯地址': [
    ...
    saveDraftForm(type) {
      const params = {
        ...
        inData: {
          ...
          bizStateName: '草稿',
          bizCode: this.$utils.getBizCode().branch,
          name: this.name,
          zgdw: '市场监督局',
          businessType: '1',
          draftData: this.form,
      ...
      this.$api.startJob(params).then(res => {
        if (res.state === 'successful') { // 跳转到下一页
          if (type === 'draft') {
            ...
            this.$router.push('/report/branchProperty?draftNumber=' + res.outData.draftNumber + '&pageOrigin=' + this.$route.query.pageOrigin)
          }
        } else {
